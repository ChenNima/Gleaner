...
export default function FilePage() {
  ...
  useEffect(() => {
    ...
    useAppStore.getState().setLastFileRoute(`/repo/${owner}/${name}/${filePath}`);
    setLoading(true);
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Breadcrumb bar */}
      <div className="flex items-center h-10 md:h-8 px-3 md:px-2 border-b bg-background/80 shrink-0 gap-1.5 md:gap-1 text-xs">
        <button
          onClick={() => window.history.back()}
          className="p-1.5 md:p-0.5 rounded hover:bg-accent text-muted-foreground hover:text-foreground shrink-0"
          title={t('file.goBack')}
        >
        ...
        <div className="flex items-center gap-1 md:gap-0.5 ml-1 min-w-0 overflow-x-auto">
          <button
            onClick={() => handleBreadcrumbClick('')}
            className="text-muted-foreground hover:text-foreground hover:underline truncate shrink-0 max-w-[120px] md:max-w-none text-xs"
          >
            {name}
          </button>
          {pathParts.map((part, i) => {
            ...
            return (
              <span key={segmentPath} className="flex items-center gap-1 md:gap-0.5 min-w-0">
